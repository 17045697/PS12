...
public class DBHelper extends SQLiteOpenHelper {
	...
	public void deleteTask(int id){
		String deleteQuery = "DELETE  FROM " + TABLE_TASK  + " WHERE " + COLUMN_ID + " = " + id;
		SQLiteDatabase db = this.getWritableDatabase();
		db.execSQL(deleteQuery);
	}
	public void updateTask(Task task){

		String updateQuery = "UPDATE" + TABLE_TASK + " SET " + COLUMN_TASK_NAME + " = '" + task.getName() + "', " + COLUMN_TASK_DESCRIPTION + " = '" + task.getDescription() + "' "  + " WHERE " + COLUMN_ID + " = '" + task.getId() + "';";
		SQLiteDatabase db = this.getWritableDatabase();
